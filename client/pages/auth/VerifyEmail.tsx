...
export default function VerifyEmail() {
  ...
  async function onVerify(e: React.FormEvent) {
    ...
    try {
      ...
      userStore.save({ id: uid, first: pending.first, last: pending.last, email: pending.email || email, phone: pending.phone, role: "citizen" });
  ...
  async function resend() {
    try {
      const pendingRaw = localStorage.getItem("app:pendingUser");
      const pending = pendingRaw ? (JSON.parse(pendingRaw) as { first?: string; last?: string }) : {};
      const res = await fetch("/api/auth/send-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, name: `${pending.first || ""} ${pending.last || ""}`.trim() }),
      });
  ...
  return (
    <FormScreen title="Verify your email" progress={60}
      bottom={<p className="text-xs text-muted-foreground">Enter the 6‑digit code sent to your email.</p>}
    >
      <form onSubmit={onVerify} className="grid gap-4">
        ...
        <Button type="submit" className="h-12 rounded-full" disabled={otp.length !== 6 || loading}>Verify</Button>
        <button type="button" className="text-sm underline justify-self-start" onClick={resend} disabled={loading}>Resend code</button>
      </form>
    </FormScreen>
  );
}
