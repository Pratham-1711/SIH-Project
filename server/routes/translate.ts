...
export const translate: RequestHandler = async (req, res) => {
  try {
    ...
    if (!key) return res.status(501).json({ error: "GOOGLE_API_KEY not configured" });
    const { q, target } = req.body as { q?: string[]; target?: string };
    if (!Array.isArray(q) || !target) return res.status(400).json({ error: "Missing q or target" });
    ...
    if (!resp.ok) {
      ...
      return res.status(500).json({ error: `Translate API ${resp.status}`, details: txt?.slice(0, 200) });
    }
